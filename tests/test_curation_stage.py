"""Curation sub-stage (exclusions → metadata) under the `curating` status, and the homepage facets."""
...
async def test_stage_lifecycle(crawler_client):
    ...
    assert (await c.post("/api/collections/ex.org/stage", json={"stage": "exclusions"})).status_code == 200
...
async def test_dashboard_stage_and_flag_facets(crawler_client):
    c = crawler_client
    await setup(c)  # ex.org: curating / exclusions
    ...
    assert 'name="stage" value="exclusions"' in home and 'name="flag" value="needs_recuration"' in home
    assert '<span id="f-stage-exclusions" class="cnt">1</span>' in home
    assert '<span id="f-stage-metadata" class="cnt">1</span>' in home
    assert '<span id="f-flag-needs_recuration" class="cnt">1</span>' in home

    rows = (await c.get("/rows?stage=exclusions")).text
    ...
    assert "0 of 3" in (await c.get("/rows?flag=needs_recuration&stage=exclusions")).text
