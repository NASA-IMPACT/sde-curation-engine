...
- **✨ Suggest metadata** — **one call per included delta URL with the full page text**, up to
  ...
  are the multi-megabyte `/code/all/…` listing URLs (~780K tokens), which belong under an exclude
  anyway. Regenerate duplicate titles fits its group calls the same way, cutting only the longest
  pages. Only the shared system prompt is written to OpenAI's
  prompt cache (`OPENAI_PROMPT_CACHE=system`): page text is unique per call, so under the default
  implicit caching every page token was billed as a cache write that was never read back. **Every page gets every field** — a title, a division and a
