...
`preflight → stage → wipe → write → validating`. When it finishes, the
page shows a **Prod run** line from test run `<id>` and **Published** `… wiped · N written (N from S3
vectors · 0 from the test index · 0 from prod)`, and a **Validation** row (`pass` via direct, N / N
...
with nothing changed, again reports `N wiped · N written`: every publish replaces the collection.
